...
public abstract class AbstractGerritSCMSource extends AbstractGitSCMSource {
            ...
            for (final Map.Entry<String, ObjectId> ref : filteredRefs.entrySet()) {
              ...
              for (Entry<String, PendingCheckInfo> check :
                ...
                try {
                  ...
                  listener
                      .getLogger()
                      .format("Unable to set checks status of %s to 'SCHEDULED'.", refKey);
                }
              }
            }
          }
  ...
  private GerritChecksApi getGerritChecksApi() throws IOException {
    try {
      ...
      UsernamePasswordCredentialsProvider.UsernamePassword credentials =
          new UsernamePasswordCredentialsProvider(getCredentials())
              .getUsernamePassword(gerritUri.getRemoteURI());
      GerritChecksApi gerritChecksApi = new GerritChecksApi(getGerritURI().getApiURI());
      if (credentials.username != null) {
        gerritChecksApi.setBasicAuthCredentials(credentials.username, credentials.password);
      }
  ...
  private HashMap<String, PendingChecksInfo> getChangesWithPendingChecks(
    ...
    GerritChecksApi checksApi = getGerritChecksApi();
