        ...
        new Retriever<Object>() {
          ...
          public Object run(
            ...
            try (RevWalk walk = new RevWalk(repository);
              ...
              Map<String, ObjectId> remoteBranches = new HashMap<>();
              if (context.wantBranches()) {
                remoteBranches =
                    client.getRemoteBranches().stream().collect(Collectors.toMap(
                        (Branch branch) -> branch.getName(),
                        (Branch branch) -> branch.getSHA1()));
                discoverBranches(repository, walk, request, remoteBranches);
              }
              if (context.wantTags()) {
                // TODO
              }
  ...
  public SCM build(@NonNull SCMHead head,
    ...
    if(getChangeRefMatcher(head.getName()).matches()) {
      ...
      UserRemoteConfig changeRemoteConfig = new UserRemoteConfig(
          ...
          "refs/changes/" + head.getName() + ":refs/remotes/" + headRemoteConfig.getName() + "/" + head.getName(),
      ...
      return new GitSCM(userRemoteConfigs, branches, false, submoduleCfg, browser,
          gitTool, Arrays.asList(new GerritFetchChangeSCMExtension(changeRemoteConfig)));
    } else {
