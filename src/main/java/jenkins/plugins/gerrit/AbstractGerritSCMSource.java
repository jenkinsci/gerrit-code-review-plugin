...
public abstract class AbstractGerritSCMSource extends AbstractGitSCMSource {
  ...
  private Map<String, ObjectId> filterRemoteReferences(
      Map<String, ObjectId> gitRefs, GerritSCMSourceRequest request) {
    ...
    try {
      ...
      try {
        if (head == null) {
          ...
          fetchRefSpecs = Arrays.asList(new RefSpec("+" + remoteRef + ":" + localRef));
        }
  ...
  private Stream<RefSpec> changeQueryToRefSpecs(QueryRequest changeQuery) throws RestApiException {
    return changeQuery
        ...
        .map(
            (ChangeInfo change) -> {
              // Force fetch 45/12345/6 into refs/remotes/origin/C-12345 to match the inferred
              // source branch
  ...
  protected void decorate(GitSCMBuilder<?> builder) {
    String refName =
        builder.head() instanceof ChangeSCMHead
            ? ((ChangeSCMHead) builder.head()).getOriginName()
