...
public abstract class AbstractGerritSCMSource extends AbstractGitSCMSource {
  ...
  protected <T, C extends GitSCMSourceContext<C, R>, R extends GitSCMSourceRequest> T doRetrieve(
    ...
    try {
      ...
      try {
        if(head == null) {
          Stream<RefSpec> refSpecs = context.asRefSpecs()
              .stream().filter((RefSpec refSpec) -> !refSpec.getSource().contains(R_CHANGES));
          ...
          fetchRefSpecs = Arrays.asList(new RefSpec(R_CHANGES + head.getName() +
              ":refs/remotes/origin/" + head.getName()));
        } else {
