        ...
        .ifPresent(
            projectEvent -> {
              ...
              try (ACLContext acl = ACL.as(ACL.SYSTEM)) {
                List<WorkflowMultiBranchProject> jenkinsItems = getJenkinsInstance()
                ...
                if(jobName != null && jenkinsItems.size() != 1) {
                  throw new IllegalStateException(
                          String.format("Project '%s' not found, not visible by user '%s' or not a multi-branch pipeline",
                          jobName, username));
                }
                      ...
                      if (scmSource instanceof GerritSCMSource) {
                        ...
                        if (gerritSCMSourceApiKey != null) {
                          if(!gerritSCMSourceApiKey.equals(apiKey)) {
                            throw new IllegalStateException("Unable to trigger the SCM source because of the ApiKey provided in gerrit web hook is invalid");
                          }
                        }
