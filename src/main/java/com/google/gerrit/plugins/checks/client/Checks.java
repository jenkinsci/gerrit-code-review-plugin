    ...
    try {
      for (long delayMillis : terminalReadbackDelaysMillis) {
        ...
        try {
          ...
          if (matchesAfterAmbiguousWrite(observed, desired)) {
            return observed;
          }
    ...
    try (CloseableHttpResponse response = client.execute(request)) {
      ...
      if (statusCode == HttpStatus.SC_OK || statusCode == HttpStatus.SC_CREATED) {
        try {
          ...
          if (parsed == null) {
            throw new IllegalStateException("Successful check POST response had no check");
          }
          return parsed;
      ...
      throw new RestApiException(
          String.format("Check POST returned definitive status %d", statusCode));
    } catch (RestApiException e) {
      throw e;
