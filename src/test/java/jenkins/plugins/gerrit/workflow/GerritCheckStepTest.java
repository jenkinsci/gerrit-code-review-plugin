...
public class GerritCheckStepTest {
  ...
  public void gerritCheckStepTestProducesUtcTimestamps() throws Exception {
    ...
    g.getClient()
        .when(
            HttpRequest.request(expectedUrl).withMethod("POST"))
        .respond(
            HttpResponse.response()
                .withStatusCode(200)
                .withBody(JsonBody.json(Collections.emptyMap())));
    ...
    g.getClient().verify(HttpRequest.request(expectedUrl), VerificationTimes.once());
    // FIXME: it's very difficult to run verifications on the request body using verify(), but we can extract and parse the requests as JSON
    ...
    assertThat("Timestamp should be in SQL format", startedAt, matchesPattern("\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}\\.\\d{3}"));

    // Whatever timestamp was formatted, validate that when it is parsed as a UTC time, it represents current time, +/- 10 seconds
    long parsedEpoch = instantFromUtc(startedAt).getEpochSecond();
    long nowEpoch = Instant.now().getEpochSecond();
    assertThat("Timestamp parsed as UTC should match current time.", Math.abs(nowEpoch - parsedEpoch), lessThanOrEqualTo(10L));
  }
  ...
  private JsonElement requestBodyJson(String requestUrl) {
    ...
    String request = g.getClient().retrieveRecordedRequests(HttpRequest.request(requestUrl), Format.JSON);
