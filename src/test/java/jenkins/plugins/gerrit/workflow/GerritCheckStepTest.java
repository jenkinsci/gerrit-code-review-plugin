...
public class GerritCheckStepTest {
  ...
  public void gerritCheckStepTestProducesUtcTimestamps() throws Exception {
    ...
    g.getClient().verify(HttpRequest.request(expectedUrl), VerificationTimes.once());
    // FIXME: it's very difficult to run verifications on the request body using verify(),
    ...
    assertThat("Timestamp should be in SQL format",
        startedAt, matchesPattern("\\d{4}-\\d{2}-\\d{2} \\d{2}:\\d{2}:\\d{2}\\.\\d{3}"));
    ...
    assertThat("Timestamp parsed as UTC should match current time.",
        Math.abs(nowEpoch - parsedEpoch), lessThanOrEqualTo(10L));
  }
  ...
  private JsonElement requestBodyJson(String requestUrl) {
    ...
    String request = g.getClient()
        .retrieveRecordedRequests(HttpRequest.request(requestUrl), Format.JSON);
    JsonElement el = gson.fromJson(request, JsonElement.class);
    String innerBody = el.getAsJsonArray().get(0)
