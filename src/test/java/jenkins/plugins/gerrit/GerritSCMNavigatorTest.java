...
public class GerritSCMNavigatorTest {
  ...
  public void sourcesShareCommonPropertiesWithTheNavigator()
    ...
    SCMSource scmSource = sourceObserver.addedSourceByProjectName.get(project.id);
    assertSCMSourceValidity(scmSource, project.id, navigator);
    GerritSCMSource gerritSCMSource = (GerritSCMSource) scmSource;
    assertEquals(1, gerritSCMSource.getTraits().size());
    assertEquals(trait, gerritSCMSource.getTraits().get(0));
  }
