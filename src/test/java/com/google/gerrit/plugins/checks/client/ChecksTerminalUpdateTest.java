...
public class ChecksTerminalUpdateTest {
  ...
  @Test
  public void interruptionAfterAReadStopsBeforeTheNextRead() throws Exception {
    CheckInput desired = terminalInput(new Timestamp(1_000L));
    FakeChecks checks = new FakeChecks(0L, 0L);
    checks.reads.add(null);
    checks.reads.add(matchingInfo(desired, desired.finished));
  ...
  @Test
  public void noContentIsSuccessfulWithoutReadback() throws Exception {
    CheckInput desired = terminalInput(new Timestamp(1_000L));
    expectPost(204, null);

    try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
      assertNull(newChecks(client, 0L).updateTerminal(desired));
    }
  ...
  @Test
  public void unlistedTwoHundredStatusIsDefinitiveAndNotReadBack() throws Exception {
    assertDefinitiveStatusIsNotReadBack(203, "unexpected success body");
  }
  ...
  public void postUsesUtf8JsonBody() throws Exception {
    ...
    HttpRequest expectedRequest =
        HttpRequest.request(checksCollectionPath())
            .withMethod("POST")
  ...
  @Test
  public void definitiveFailureTextOmitsRequestAndResponseDetails() throws Exception {
    CheckInput desired = terminalInput(new Timestamp(1_000L));
    desired.message = "request-body-secret";
    desired.url = "https://request-secret.example/job/1";
    expectPost(400, "response-body-secret https://response-secret.example/result");

    RestApiException failure;
    try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
      try {
    ...
    String text = exceptionText(failure);
    assertFalse(text.contains("request-body-secret"));
    assertFalse(text.contains("request-secret.example"));
    assertFalse(text.contains("response-body-secret"));
    assertFalse(text.contains("response-secret.example"));
    assertTrue(text.contains("400"));
    verifyOnePostAndGetCount(0);
  }

  @Test
  public void ambiguousFailureTextOmitsResponseDetails() throws Exception {
    CheckInput desired = terminalInput(new Timestamp(1_000L));
    expectPost(504, "response-body-secret https://response-secret.example/result");
    expectReadback(404, null);

    RestApiException failure;
    try (CloseableHttpClient client = HttpClientBuilder.create().build()) {
      try {
        newChecks(client, 0L).updateTerminal(desired);
        fail("Expected unreconciled ambiguous failure");
        return;
      } catch (RestApiException expected) {
        failure = expected;
      }
